import { useEffect, useRef, useCallback, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { fetchProducts } from "../../redux/slices/productSlice";
import { Link } from "react-router-dom";

export default function ProductsPage() {
  const dispatch = useDispatch();
  const { list: products, totalPages, error } = useSelector(state => state.products);
  const { search, category } = useSelector(state => state.filters);

  const [page, setPage] = useState(1);
  const observer = useRef();

  useEffect(() => {
    setPage(1);
    dispatch(fetchProducts({
      search: search.trim(),
      category: category || "",
      page: 1,
      limit: 12
    }));
  }, [dispatch]);

  useEffect(() => {
    if (page > 1) {
      dispatch(fetchProducts({
        search: search.trim(),
        category: category || "",
        page,
        limit: 12
      }));
    }
  }, [dispatch, page, search, category]);

  const lastProductRef = useCallback(
    (node) => {
      if (observer.current) observer.current.disconnect();

      observer.current = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && page < totalPages) {
          setPage(prev => prev + 1);
        }
      });

      if (node) observer.current.observe(node);
    },
    [page, totalPages]
  );

  const getImageSrc = (img) => {
    if (!img) return "https://via.placeholder.com/200";
    const normalized = img.replace(/\\/g, "/");
    if (normalized.startsWith("http")) return normalized;
    const path = normalized.startsWith("uploads/") ? normalized : `uploads/${normalized}`;
    return `http://localhost:5000/${path}`;
  };

  return (
    <div className="container py-4">
      <h2 className="mb-4 text-center fw-bold">Our Products</h2>

      {error && <p className="text-center text-danger">{error}</p>}

      <div className="row justify-content-center g-4">
        {products.length === 0 ? (
          <div className="col-12 text-center text-muted">No products found</div>
        ) : (
          products.map((p, idx) => {
            const isLast = products.length === idx + 1;
            return (
              <div
                key={p._id}
                className="col-12 col-sm-6 col-md-4 col-lg-3"
                ref={isLast ? lastProductRef : null}
              >
                <ProductCard p={p} getImageSrc={getImageSrc} />
              </div>
            );
          })
        )}
      </div>
    </div>
  );
}

function ProductCard({ p, getImageSrc }) {
  return (
    <div className="card shadow-sm h-100 border-0">
      <Link to={`/client/product/${p._id}`} className="text-decoration-none text-dark">
        <img
          src={getImageSrc(p.images?.[0])}
          className="card-img-top p-3"
          alt={p.name}
          style={{ height: "200px", objectFit: "contain" }}
        />
      </Link>
      <div className="card-body text-center">
        <Link to={`/client/product/${p._id}`} className="text-decoration-none text-dark">
          <h5 className="card-title">{p.name}</h5>
        </Link>
        <p className="fw-semibold fs-5 text-primary mb-2">₹{p.price}</p>
      </div>
      <ul className="list-group list-group-flush">
        <li className="list-group-item">
          <strong>Stock:</strong> {p.stock ?? "Out of stock"}
        </li>
        <li className="list-group-item">
          {p.description.length > 70 ? (
            <small>{p.description.slice(0, 70)}...</small>
          ) : (
            <small>{p.description}</small>
          )}
        </li>
      </ul>
    </div>
  );
}
